...
pub fn map_to_scalar(hash: &sha256::Hash) -> Scalar {
    Scalar::random(&mut ChaChaRng::from_seed(*hash.as_byte_array()))
}

fn map_to_g1(hash: &sha256::Hash) -> G1Projective {
    G1Projective::random(&mut ChaChaRng::from_seed(*hash.as_byte_array()))
}

fn map_to_g2(hash: &sha256::Hash) -> G2Projective {
    G2Projective::random(&mut ChaChaRng::from_seed(*hash.as_byte_array()))
}
